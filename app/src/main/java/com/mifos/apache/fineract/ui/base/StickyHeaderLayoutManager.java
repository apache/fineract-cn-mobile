...
import com.mifos.apache.fineract.ui.adapters.SectioningAdapter;
...
public class StickyHeaderLayoutManager extends RecyclerView.LayoutManager {
     ...
     * @see HeaderPosition
     */
    public void setHeaderPositionChangedCallback(HeaderPositionChangedCallback
    ...
    public void onAdapterChanged(RecyclerView.Adapter oldAdapter, RecyclerView.Adapter newAdapter) {
        ...
        try {
            ...
            throw new ClassCastException(
                    "StickyHeaderLayoutManager must be used with a RecyclerView where the adapter"
							+ " is a kind of SectioningAdapter");
        }
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (state instanceof SavedState) {
            ...
            Log.e(TAG, "onRestoreInstanceState: invalid saved state class, expected: "
                    + SavedState.class.getCanonicalName() + " got: "
    ...
    private View createSectionHeaderIfNeeded(RecyclerView.Recycler recycler, int sectionIndex) {
        ...
        if (adapter.doesSectionHaveHeader(sectionIndex)) {
            ...
            for (int i = 0, n = getChildCount(); i < n; i++) {
                ...
                if (getViewBaseType(view) == SectioningAdapter.TYPE_HEADER && getViewSectionIndex(
                        view) == sectionIndex) {
                    return view;
                }
            }
        ...
        if (dy < 0) {
            ...
            while (scrolled > dy) {
                ...
                if (firstViewAdapterPosition > 0 && scrolled > dy) {
                    ...
                    // we're skipping headers. they should already be vended, but if we're
					// vending a ghostHeader
            ...
            while (scrolled < dy) {
                ...
                if (scrolled < dy && nextAdapterPosition < state.getItemCount()) {
                    ...
                    if (itemViewType == SectioningAdapter.TYPE_HEADER) {
                        ...
                        // get the header and measure it so we can followup immediately by
						// vending the ghost header
    ...
    /**
     * @param fullyVisibleOnly if true, the search will be limited to the first item not hanging off
     *                         top of screen or partially obscured by a header
     * @return the viewholder for the first visible item (not header or footer)
     */
    @Nullable
    public SectioningAdapter.ItemViewHolder getFirstVisibleItemViewHolder(boolean
            fullyVisibleOnly) {
    ...
    /**
     * @param fullyVisibleOnly if true, the search will be limited to the first header not hanging
     *                         off top of screen
     * @return the viewholder for the first visible header (not item or footer)
     */
    @Nullable
    public SectioningAdapter.HeaderViewHolder getFirstVisibleHeaderViewHolder(boolean
            fullyVisibleOnly) {
        return (SectioningAdapter.HeaderViewHolder) getFirstVisibleViewHolderOfType(
                SectioningAdapter.TYPE_HEADER, fullyVisibleOnly);
    }

    /**
     * @param fullyVisibleOnly if true, the search will be limited to the first footer not hanging
     *                         off top of screen or partially obscured by a header
     * @return the viewholder for the first visible footer (not header or item)
     */
    @Nullable
    public SectioningAdapter.FooterViewHolder getFirstVisibleFooterViewHolder(boolean
            fullyVisibleOnly) {
        return (SectioningAdapter.FooterViewHolder) getFirstVisibleViewHolderOfType(
                SectioningAdapter.TYPE_FOOTER, fullyVisibleOnly);
    }

    @Nullable
    private SectioningAdapter.ViewHolder getFirstVisibleViewHolderOfType(int baseType,
    ...
    private void recycleViewsOutOfBounds(RecyclerView.Recycler recycler) {
        ...
        for (int i = 0; i < numChildren; i++) {
            ...
            if (getViewBaseType(view) == SectioningAdapter.TYPE_HEADER
                    && !remainingSections.contains(sectionIndex)) {
                float translationY = view.getTranslationY();
                if ((getDecoratedBottom(view) + translationY) < 0 || (getDecoratedTop(view)
                        + translationY) > height) {
                    viewsToRecycle.add(view);
    ...
    private void recordHeaderPositionAndNotify(int sectionIndex, View headerView,
    ...
    private class SmoothScroller extends LinearSmoothScroller {
        ...
        SmoothScroller(Context context, int distanceInPixels) {
            ...
            float millisecondsPerPx = calculateSpeedPerPixel(
    ...
    private static class SavedState implements Parcelable {
        ...
        public String toString() {
            return "<" + this.getClass().getCanonicalName() + " firstViewAdapterPosition: "
                    + firstViewAdapterPosition + " firstViewTop: " + firstViewTop + ">";
        }
