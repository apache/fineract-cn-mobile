...
/**
 * SectioningAdapter
 * Represents a list of sections, each containing a list of items and optionally a header and or
 * footer item.
 * SectioningAdapter may be used with a normal RecyclerView.LinearLayoutManager but is meant for use
 * with
 * StickyHeaderLayoutManager to allow for sticky positioning of header items.
 * <p/>
 * When invalidating the adapter's contents NEVER use RecyclerView.Adapter.notify* methods. These
 * methods
 ...
 * <p/>
 * SectioningAdapter manages four types of items: TYPE_HEADER, TYPE_ITEM, TYPE_FOOTER and
 * TYPE_GHOST_HEADER.
...
public class SectioningAdapter extends RecyclerView.Adapter<SectioningAdapter.ViewHolder> {
    ...
    /**
     * For scenarios with multiple types of headers, override this to return an integer in range
     * [0,255] specifying a custom type for this header.
     * The value you return here will be passes to onCreateHeaderViewHolder and
     * onBindHeaderViewHolder as the 'userType'
     *
    ...
    /**
     * For scenarios with multiple types of footers, override this to return an integer in range
     * [0,
     * 255] specifying a custom type for this footer.
     * The value you return here will be passes to onCreateFooterViewHolder and
     * onBindFooterViewHolder as the 'userType'
     *
    ...
    /**
     * For scenarios with multiple types of items, override this to return an integer in range
     * [0,255] specifying a custom type for the item at this position
     * The value you return here will be passes to onCreateItemViewHolder and onBindItemViewHolder
     * as the 'userType'
     *
     ...
     *                     an adapter position.
     * @param itemUserType If getSectionItemUserType is overridden to vend custom types, this will
     *                     be the specified type
     ...
     * @param headerUserType If getSectionHeaderUserType is overridden to vend custom types, this
     *                       will be the specified type
     ...
     * @param parent         The ViewGroup into which the new View will be added after it is bound
     *                       to an adapter position.
     * @param footerUserType If getSectionHeaderUserType is overridden to vend custom types, this
     *                       will be the specified type
     ...
     * @param itemUserType if getSectionItemUserType is overridden to provide custom item types,
     *                     this will be the type for this item
     */
     ...
     * @param headerUserType if getSectionHeaderUserType is overridden to provide custom header
     *                       types, this will be the type for this header
     */
     ...
     * @param footerUserType if getSectionFooterUserType is overridden to provide custom footer
     *                       types, this will be the type for this footer
     */
    public void onBindFooterViewHolder(FooterViewHolder viewHolder, int sectionIndex, int
            footerUserType) {
    }
    ...
    public int getSectionForAdapterPosition(int adapterPosition) {
        ...
        if (adapterPosition < 0 || adapterPosition >= getItemCount()) {
            throw new IndexOutOfBoundsException("adapterPosition " + adapterPosition
    ...
    public int getPositionOfItemInSection(int sectionIndex, int adapterPosition) {
        ...
        if (localPosition > section.length) {
            throw new IndexOutOfBoundsException(
                    "adapterPosition: " + adapterPosition + " is beyond sectionIndex: "
                            + sectionIndex + " length: " + section.length);
        }
     ...
     *
     * @param sectionIndex the index of the section
     * @return adapter position of that section's ghost header, or NO_POSITION if section has no
     * ghost header
     */
    ...
    public int getAdapterPositionForSectionItem(int sectionIndex, int offsetIntoSection) {
        if (doesSectionHaveHeader(sectionIndex)) {
            return getAdapterPosition(sectionIndex, offsetIntoSection) + 2; // header is at
            // position 0, ghostHeader at position 1
        } else {
     ...
     *
     * @param sectionIndex the index of the section
     * @return adapter position of that section's footer, or NO_POSITION if section does not have
     * footer
     */
    ...
    public void clearSelection(boolean notify) {
        ...
        HashMap<Integer, SectionSelectionState> selectionState = notify ? new HashMap<>(
                selectionStateBySection) : null;
    ...
    /**
     * Walks the selection state of the adapter, in reverse order from end to front. This is to
    ...
    public void toggleSectionItemSelected(int sectionIndex, int itemIndex) {
        setSectionItemSelected(sectionIndex, itemIndex,
    ...
    private void notifySectionItemRangeInserted(int sectionIndex, int fromPosition, int number,
            boolean updateSelectionState) {
        if (sections == null) {
            buildSectionIndex();
            notifyAllSectionsDataSetChanged();
        } else {
            ...
            if (fromPosition > section.numberOfItems) {
                throw new IndexOutOfBoundsException("itemIndex adapterPosition: " + fromPosition
                        + " exceeds sectionIndex numberOfItems: " + section.numberOfItems);
            }
    ...
    private void notifySectionItemRangeRemoved(int sectionIndex, int fromPosition, int number,
            boolean updateSelectionState) {
        if (sections == null) {
            buildSectionIndex();
            notifyAllSectionsDataSetChanged();
            ...
            if (fromPosition + number > section.numberOfItems) {
                throw new IndexOutOfBoundsException(
                        "itemIndex adapterPosition: " + fromPosition + number
                                + " exceeds sectionIndex numberOfItems: " + section.numberOfItems);
            }
     ...
     * @param itemIndex    the index of the item relative to the section (where 0 is the first item
     *                     in the section)
     */
    public void notifySectionItemChanged(int sectionIndex, int itemIndex) {
        if (sections == null) {
            ...
            if (itemIndex >= section.numberOfItems) {
                throw new IndexOutOfBoundsException("itemIndex adapterPosition: " + itemIndex + " "
                        + "exceeds sectionIndex numberOfItems: " + section.numberOfItems);
            }
            if (section.hasHeader) {
                itemIndex += 2;
        ...
        if (sections == null) {
            ...
            if (!section.hasFooter) {
                throw new IllegalArgumentException(
                        "notifySectionFooterInserted: adapter implementation reports that section "
    ...
    public void notifySectionFooterRemoved(int sectionIndex) {
        if (sections == null) {
            ...
            if (section.hasFooter) {
                throw new IllegalArgumentException(
                        "notifySectionFooterRemoved: adapter implementation reports that section "
    ...
    public void notifySectionFooterChanged(int sectionIndex) {
        if (sections == null) {
            ...
            if (!section.hasFooter) {
                throw new IllegalArgumentException(
                        "notifySectionFooterChanged: adapter implementation reports that section "
                                + sectionIndex + " does not have a footer");
            }
    ...
    private void updateSectionItemRangeSelectionState(int sectionIndex, int fromPosition,
            int delta) {
    ...
    private void updateCollapseAndSelectionStateForSectionChange(int sectionIndex, int delta) {
        ...
        HashMap<Integer, SectionSelectionState> selectionState = new HashMap<>(
                selectionStateBySection);
        selectionStateBySection.clear();
    ...
    public int getItemViewType(int adapterPosition) {
        ...
        if (adapterPosition < 0) {
            ...
            throw new IndexOutOfBoundsException(
                    "adapterPosition (" + adapterPosition + ")  cannot be > getItemCount() ("
                            + getItemCount() + ")");
        }
     ...
     * @return the base type (TYPE_HEADER, TYPE_GHOST_HEADER, TYPE_ITEM, TYPE_FOOTER) of the item at
     * a given adapter position
     */
    ...
    public static int unmaskBaseViewType(int itemViewTypeMask) {
        return itemViewTypeMask
                & 0xFF; // base view type (HEADER/ITEM/FOOTER/GHOST_HEADER) is lower 8 bits
    }
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        throw new IndexOutOfBoundsException("unrecognized viewType: " + viewType
    ...
    public void onBindViewHolder(ViewHolder holder, int adapterPosition) {
        ...
        switch (baseType) {
            ...
            default:
                throw new IllegalArgumentException("unrecognized viewType: " + baseType
                        + " does not correspond to TYPE_ITEM, "
